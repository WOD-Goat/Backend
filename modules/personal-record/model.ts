...
import { PersonalRecordData, PersonalRecordEntry, PersonalRecordEntry_Legacy } from '../../types/personalrecord.types';
...
/**
    ...
    class PersonalRecord {
             ...
             */
            static async updateHistoryEntry(userId: string, exerciseId: string, entryIndex: number, updatedEntry: any): Promise<void> {
                try {
                    ...
                    if (!docSnap.exists) throw new Error('Personal record document not found');
                    const data = docSnap.data();
                    if (!data || !Array.isArray(data.history)) throw new Error('No history array found');
                    const history = [...data.history];
                    if (entryIndex < 0 || entryIndex >= history.length) throw new Error('Invalid history entry index');
             ...
             * @param entryIndex Index of the entry in history array
             */
            static async deleteHistoryEntry(userId: string, exerciseId: string, entryIndex: number): Promise<void> {
                try {
                    const docRef = firestore
                        .collection('users')
                        .doc(userId)
                ...
                if (docSnap.exists) {
                    ...
                    if (history.length > 0) {
                        ...
                        const sorted = [...history].sort((a, b) => {
                            const aValue = a.bestActual1RM ?? a.bestEstimated1RM ?? a.bestWeight ?? a.bestReps ?? a.bestTimeInSeconds ?? 0;
                            const bValue = b.bestActual1RM ?? b.bestEstimated1RM ?? b.bestWeight ?? b.bestReps ?? b.bestTimeInSeconds ?? 0;
                    ...
                    if (!bestPR) {
                        ...
                        const newValue = this.bestActual1RM ?? this.bestEstimated1RM ?? this.bestWeight ?? this.bestReps ?? this.bestTimeInSeconds ?? 0;
                        const bestValue = bestPR.bestActual1RM ?? bestPR.bestEstimated1RM ?? bestPR.bestWeight ?? bestPR.bestReps ?? bestPR.bestTimeInSeconds ?? 0;
         ...
         */
        static async update(userId: string, exerciseId: string, updateData: Partial<PersonalRecordEntry_Legacy>): Promise<void> {
            try {
         ...
         */
        static calculateImprovement(current: PersonalRecordEntry, previous: PersonalRecordEntry | null): number | null {
            ...
            } else if (current.bestTimeInSeconds !== null && previous.bestTimeInSeconds !== null) {
         ...
         */
        static async getLatestByExercise(userId: string, exerciseId: string, count: number = 5): Promise<PersonalRecordEntry[]> {
            try {
                const doc = await firestore
                    .collection('users')
                    .doc(userId)
