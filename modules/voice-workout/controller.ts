...
import { parseWorkoutFromAudio, formatWorkoutFromAudio } from "./gemini.service";
...
class VoiceWorkoutController {
  ...
  static async formatVoiceWorkout(
    req: AuthenticatedRequest,
    res: Response,
  ): Promise<void> {
    try {
      ...
      const workoutDraft = await formatWorkoutFromAudio(
        audioBuffer,
        geminiMimeType,
        todayISO,
      );

      if (workoutDraft.wods.length === 0) {
        res.status(422).json({
          success: false,
          message:
            "No workout sections detected in the audio. Try describing your workout clearly.",
          data: { transcript: workoutDraft.transcript },
        });
        return;
      }
      ...
      console.error("Free-form workout formatting error:", error);
      const message = error instanceof Error ? error.message : "Unknown error";
      ...
      const isUserFacingError =
        message.includes("Audio was blocked") ||
        message.includes("Failed to format") ||
        message.includes("silent or corrupted");
